...
export default function ConfirmationPrompt({ slot, onConfirm, onCancel }: Props) {
    ...
    return (
        ...
        >
            ...
            <p className="text-sm mb-4">A <span className="font-semibold text-mint-600">$20 refundable hold</span> will
                be placed.</p>
            <div className="flex space-x-3">
                <ContinueButton onClick={onConfirm}>Confirm & Pay</ContinueButton>
                <button onClick={onCancel}
                        className="flex-1 py-2 rounded-lg bg-white/20 backdrop-blur-md text-mint-600">Cancel
                </button>
            </div>
        </motion.div>
    );
